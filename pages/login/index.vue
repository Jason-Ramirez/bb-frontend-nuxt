<script>
  ...
  //   //   })
  //   // } catch (error) {
  //   //   console.log("ERROR: this.$auth.loginWith('laravelSanctum', {", error);
  //   // }
  ...
  export default defineNuxtComponent({
    setup() {
      ...
      async function onLogin(e) {
        ...
        //   await csrf();
        //   const loginResponse = await this.$auth.loginWith('laravelSanctum', { data: credentials });
        //   // this.$router.push({ path: '/' }); // this is working here
        //   console.log('login component loginResponse', loginResponse);
        // } catch (error) {
        //   console.log('error 1', error)
        // }
        // useAuth.login(credentials);
        
        const loader = this.$loading.show({ loader: 'bars', color: '#0365cd' }, { default: () => h(GridLoader, { color: '#0365cd' }), });

        try {
          // ATTEMPT 1
          await this.$auth.loginWith('laravelSanctum', { data: credentials }).then(() => {
            const user = this.$auth.user;

            // const useUser = useState('user', () => user);
            // console.log('useUser', useUser, useUser.value)
            
            // const localUser = localStorage.getItem('user');
            // console.log('localUser', localUser, localStorage)

            console.log('Login: this.$auth.user;', this.$auth.user);
          ...
          // ATTEMPT 3
          // let response = await this.$auth.loginWith('laravelSanctum', { data: credentials }).then(data => {
          //   console.log('local then', data);
          // })
